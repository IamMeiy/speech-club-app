<div class="w-full max-w-4xl mx-auto space-y-6" x-data="{
    ...
    <form wire:submit="save" class="w-full space-y-6">
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    <div class="flex items-center gap-2">
                        <h1 class="text-xl sm:text-2xl font-bold tracking-tight text-slate-900 dark:text-white">Edit Meeting #{{ $meeting->meeting_number }}</h1>
                        <span class="text-[11px] font-bold px-2.5 py-0.5 rounded-full bg-primary-50 dark:bg-primary-950/60 text-primary-600 dark:text-primary-400 border border-primary-200/60 dark:border-primary-900/60"
                              x-text="'Step ' + step + ' of 3'"></span>
                    </div>
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{{ $meeting->club->name }}</p>
                </div>
            </div>

            {{-- Quick Header Actions --}}
            <div class="flex items-center gap-2.5 self-end sm:self-auto">
                <a href="{{ route('meetings.show', $meeting) }}" class="px-4 py-2 text-xs sm:text-sm font-semibold text-slate-600 dark:text-slate-300 hover:bg-white dark:hover:bg-slate-800 rounded-2xl border border-slate-200/80 dark:border-slate-800 transition-colors">
                    Cancel
                </a>
                <button type="submit" wire:loading.attr="disabled"
                        class="inline-flex items-center gap-2 px-5 py-2 bg-primary-600 hover:bg-primary-700 text-white text-xs sm:text-sm font-semibold rounded-2xl transition-all shadow-md shadow-primary-600/20 disabled:opacity-60 active:scale-[0.98]">
        ...
        <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-sm p-4 sm:p-5 transition-colors">
            <div class="max-w-xl mx-auto relative">
                {{-- Background connecting track between centers of step 1 and step 3 --}}
                <div class="absolute top-5 -translate-y-1/2 left-[16.666%] right-[16.666%] h-1 bg-slate-100 dark:bg-slate-800 -z-0"></div>
                {{-- Active filled connecting track --}}
                <div class="absolute top-5 -translate-y-1/2 left-[16.666%] h-1 bg-primary-600 transition-all duration-300 -z-0"
                     :style="'width: ' + ((step - 1) / 2 * 66.666) + '%;'"></div>

                <div class="grid grid-cols-3 relative z-10">
                    {{-- Step 1 Tab --}}
                    <button type="button" @click="goToStep(1)" class="flex flex-col items-center group focus:outline-none cursor-pointer px-1">
                        <div class="w-10 h-10 rounded-2xl flex items-center justify-center font-bold text-sm transition-all duration-200 shadow-sm"
                             :class="step === 1 
                                ? 'bg-primary-600 text-white ring-4 ring-primary-100 dark:ring-primary-950/60 shadow-primary-600/20' 
                                : (step > 1 ? 'bg-emerald-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500 hover:bg-slate-200 dark:hover:bg-slate-700')">
                        ...
                        <span class="mt-2 text-xs font-semibold transition-colors text-center"
                              :class="step === 1 ? 'text-primary-600 dark:text-primary-400 font-bold' : (step > 1 ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-500 dark:text-slate-400')">
                            Details & MoM
                        </span>
                    </button>

                    {{-- Step 2 Tab --}}
                    <button type="button" @click="goToStep(2)" class="flex flex-col items-center group focus:outline-none cursor-pointer px-1">
                        <div class="w-10 h-10 rounded-2xl flex items-center justify-center font-bold text-sm transition-all duration-200 shadow-sm"
                             :class="step === 2 
                                ? 'bg-primary-600 text-white ring-4 ring-primary-100 dark:ring-primary-950/60 shadow-primary-600/20' 
                                : (step > 2 ? 'bg-emerald-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500 hover:bg-slate-200 dark:hover:bg-slate-700')">
                        ...
                        <span class="mt-2 text-xs font-semibold transition-colors text-center"
                              :class="step === 2 ? 'text-primary-600 dark:text-primary-400 font-bold' : (step > 2 ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-500 dark:text-slate-400')">
                            Meeting Roles
                        </span>
                    </button>

                    {{-- Step 3 Tab --}}
                    <button type="button" @click="goToStep(3)" class="flex flex-col items-center group focus:outline-none cursor-pointer px-1">
                        <div class="w-10 h-10 rounded-2xl flex items-center justify-center font-bold text-sm transition-all duration-200 shadow-sm"
                             :class="step === 3 
                                ? 'bg-primary-600 text-white ring-4 ring-primary-100 dark:ring-primary-950/60 shadow-primary-600/20' 
                        ...
                        <span class="mt-2 text-xs font-semibold transition-colors text-center"
                              :class="step === 3 ? 'text-primary-600 dark:text-primary-400 font-bold' : 'text-slate-500 dark:text-slate-400'">
                            Speeches & Program
                        </span>
                    </button>
                </div>
        ...
        {{-- ================================================================ --}}
        {{-- STEP 1 CONTAINER: DETAILS, AGENDA & MINUTES OF MEETING --}}
        {{-- ================================================================ --}}
        <div x-show="step === 1" x-cloak class="w-full space-y-6">
            ...
            <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-sm p-6 sm:p-8 transition-colors">
                <div class="flex items-center gap-3 mb-6 pb-3 border-b border-slate-100 dark:border-slate-800">
                    <div class="w-9 h-9 rounded-2xl bg-primary-100 dark:bg-primary-950/80 text-primary-600 dark:text-primary-400 flex items-center justify-center font-bold">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                    </div>
                    <div>
                        <h2 class="text-base font-bold text-slate-900 dark:text-white">Meeting Information & Notes</h2>
                        <p class="text-xs text-slate-500 dark:text-slate-400">Date, number, theme, agenda notes & minutes of meeting (MoM)</p>
                    </div>
                </div>
                ...
                <div class="space-y-5">
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-1.5">
                            <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300">Minutes of Meeting (MoM)</label>
                            <span class="text-[10px] text-slate-400 font-medium">Record of discussions, decisions, officer reports & awards</span>
                        </div>
                        <x-rich-text-editor wire:model="minutes_of_meeting" placeholder="Official record of what happened during the meeting, business session, executive reports…" />
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-sm p-6 sm:p-8 transition-colors">
                <div class="flex items-center gap-3 mb-6 pb-3 border-b border-slate-100 dark:border-slate-800">
                    <div class="w-9 h-9 rounded-2xl bg-amber-500/10 dark:bg-amber-400/10 text-amber-600 dark:text-amber-400 flex items-center justify-center font-bold">
            ...
            <div class="flex items-center justify-between pt-2">
                ...
                <button type="button" @click="goToStep(2)"
                        class="inline-flex items-center gap-2 px-6 py-2.5 bg-primary-600 hover:bg-primary-700 text-white text-xs sm:text-sm font-semibold rounded-2xl transition-all shadow-md shadow-primary-600/20 active:scale-[0.98]">
                    <span>Next: Meeting Roles</span>
        ...
        {{-- ================================================================ --}}
        {{-- STEP 2 CONTAINER: MEETING ROLES --}}
        {{-- ================================================================ --}}
        <div x-show="step === 2" x-cloak class="w-full space-y-6">

            {{-- Fixed Meeting Roles --}}
            <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-sm p-6 sm:p-8 transition-colors" wire:key="fixed-roles-card-{{ $meeting->club_id }}">
                <div class="flex items-center justify-between mb-6 pb-3 border-b border-slate-100 dark:border-slate-800">
                    <div class="flex items-center gap-3">
                        <div class="w-9 h-9 rounded-2xl bg-indigo-100 dark:bg-indigo-950/80 text-indigo-600 dark:text-indigo-400 flex items-center justify-center font-bold">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/></svg>
                        </div>
                        <div>
                            <h2 class="text-base font-bold text-slate-900 dark:text-white">Meeting Roles</h2>
                            <p class="text-xs text-slate-500 dark:text-slate-400">Assign members to standard meeting role functions</p>
                        </div>
                    </div>
                    <span class="text-xs font-semibold px-2.5 py-1 rounded-full bg-indigo-50 dark:bg-indigo-950/60 text-indigo-600 dark:text-indigo-400">8 Roles</span>
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    @foreach($roleTypes as $roleType)
                    <div wire:key="role-type-{{ $roleType->id }}-{{ $meeting->club_id }}">
            ...
            <div class="flex items-center justify-between pt-2">
                <button type="button" @click="goToStep(1)"
                        class="inline-flex items-center gap-2 px-5 py-2.5 text-xs sm:text-sm text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white font-semibold transition-colors">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
                    <span>Back to Details</span>
                </button>
                <button type="button" @click="goToStep(3)"
                        class="inline-flex items-center gap-2 px-6 py-2.5 bg-primary-600 hover:bg-primary-700 text-white text-xs sm:text-sm font-semibold rounded-2xl transition-all shadow-md shadow-primary-600/20 active:scale-[0.98]">
                    <span>Next: Speeches & Program</span>
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3"/></svg>
                </button>
            </div>
        </div>{{-- End Step 2 --}}
        ...
        {{-- ================================================================ --}}
        {{-- STEP 3 CONTAINER: SPEECHES & PROGRAM --}}
        {{-- ================================================================ --}}
        <div x-show="step === 3" x-cloak class="w-full space-y-6">

            {{-- Prepared Speakers --}}
            <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-sm p-6 sm:p-8 transition-colors">
                <div class="flex items-center justify-between mb-6 pb-3 border-b border-slate-100 dark:border-slate-800">
                    <div class="flex items-center gap-3">
                        <div class="w-9 h-9 rounded-2xl bg-emerald-100 dark:bg-emerald-950/80 text-emerald-600 dark:text-emerald-400 flex items-center justify-center font-bold">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-slate-500 dark:text-slate-400">Searchable speech catalog, projects, and speeches</p>
                        </div>
                    </div>
                    <span class="text-xs font-semibold px-2.5 py-1 rounded-full bg-emerald-50 dark:bg-emerald-950/60 text-emerald-600 dark:text-emerald-400"
                          x-text="speakers.length + ' Speaker(s)'"></span>
                </div>
                ...
                <div class="space-y-4">
                    <template x-for="(speaker, index) in speakers" :key="index">
                        <div class="flex gap-3 items-start p-4 sm:p-5 bg-slate-50 dark:bg-slate-800/60 rounded-2xl border border-slate-200/60 dark:border-slate-700/50">
                            <div class="w-8 h-8 bg-emerald-100 dark:bg-emerald-950/80 text-emerald-600 dark:text-emerald-400 rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5 font-bold text-xs">
                                <span x-text="index + 1"></span>
                            </div>
                            <div class="flex-1 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3.5">
                                <div>
                                ...
                                <div class="sm:col-span-2">
                                    <label class="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1">Speech Topic</label>
                                    <input x-model="speaker.topic" type="text" placeholder="Speech title / topic"
                                ...
                                <div>
                                    <label class="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1">Project from Catalog</label>
                                    
                                    {{-- Searchable Project Catalog Combobox --}}
                                    <div class="relative" x-data="{
                                        open: false,
                                        search: '',
                                        get filtered() {
            ...
            <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-sm p-6 sm:p-8 transition-colors">
                <div class="flex items-center justify-between mb-6 pb-3 border-b border-slate-100 dark:border-slate-800">
                    <div class="flex items-center gap-3">
                        <div class="w-9 h-9 rounded-2xl bg-purple-100 dark:bg-purple-950/80 text-purple-600 dark:text-purple-400 flex items-center justify-center font-bold">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>
                        </div>
                        <div>
                            <h2 class="text-base font-bold text-slate-900 dark:text-white">Table Topics (TTM) Speakers</h2>
                            <p class="text-xs text-slate-500 dark:text-slate-400">Impromptu speaking session participants</p>
                        </div>
                    </div>
                    <span class="text-xs font-semibold px-2.5 py-1 rounded-full bg-purple-50 dark:bg-purple-950/60 text-purple-600 dark:text-purple-400"
                          x-text="ttmSpeakers.length + ' Participant(s)'"></span>
                </div>
            ...
            <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-sm p-6 sm:p-8 transition-colors">
                <div class="flex items-center justify-between mb-6 pb-3 border-b border-slate-100 dark:border-slate-800">
                    <div class="flex items-center gap-3">
                        <div class="w-9 h-9 rounded-2xl bg-cyan-100 dark:bg-cyan-950/80 text-cyan-600 dark:text-cyan-400 flex items-center justify-center font-bold">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-slate-500 dark:text-slate-400">Assign evaluators to prepared speakers</p>
                        </div>
                    </div>
                    <span class="text-xs font-semibold px-2.5 py-1 rounded-full bg-cyan-50 dark:bg-cyan-950/60 text-cyan-600 dark:text-cyan-400"
                          x-text="evaluations.length + ' Evaluator(s)'"></span>
                </div>
                ...
                <button type="button" @click="addEvaluation()"
                        class="mt-4 inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs sm:text-sm text-cyan-600 dark:text-cyan-400 bg-cyan-50 dark:bg-cyan-950/60 hover:bg-cyan-100 dark:hover:bg-cyan-900/60 font-semibold transition-colors">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 4v16m8-8H4"/></svg>
                    Add Evaluation
                </button>
            </div>
            ...
            <div class="flex items-center justify-between pt-2">
                ...
                <div class="flex items-center gap-3">
                    <a href="{{ route('meetings.show', $meeting) }}" class="px-5 py-2.5 text-xs sm:text-sm text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white font-semibold transition-colors">Cancel</a>
                    <button type="submit" wire:loading.attr="disabled"
                            class="inline-flex items-center gap-2 px-7 py-2.5 bg-primary-600 hover:bg-primary-700 text-white text-xs sm:text-sm font-semibold rounded-2xl transition-all shadow-md shadow-primary-600/20 disabled:opacity-60 active:scale-[0.98]">
                        <span wire:loading.remove>Save Changes</span>
                        <span wire:loading.inline-flex class="items-center gap-2">
                            <svg class="animate-spin h-4 w-4" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>
                            Saving…
